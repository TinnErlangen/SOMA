...
for sub in subjs:
    for run_idx,run in enumerate(runs):
        ...
        icaref = mne.preprocessing.ICA(n_components=None,max_iter=1000,method="picard",allow_ref_meg=True)
        ...
        icameg = mne.preprocessing.ICA(n_components=None,max_iter=1000,method="picard")
        picks = mne.pick_types(raw.info,meg=True,ref_meg=False)
        icameg.fit(raw,picks=picks)
        icameg.save(proc_dir+sub+"_"+run+"_prepro_meg-ica.fif")

        ica = mne.preprocessing.ICA(n_components=None,max_iter=1000,method="picard",allow_ref_meg=True)
